...
```{r}
counts = read.table("counts_0.txt", header=TRUE, row.names=1, sep="\t")
...
```{r}
coldata_s <-coldata[order(coldata$sample),]
...
#make deseq2 object
```{r}
dds = DESeqDataSetFromMatrix(countData = counts, colData = coldata, design= ~ cage + time + housing)
```
